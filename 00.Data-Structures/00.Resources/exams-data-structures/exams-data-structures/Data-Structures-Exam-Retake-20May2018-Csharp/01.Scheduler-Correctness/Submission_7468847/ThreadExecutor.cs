...
public class ThreadExecutor : IScheduler
{
    ...
    public int Cycle(int cycles)
    {
        ...
        LinkedListNode<Task> low = new LinkedListNode<Task>(new Task(5, this.cycles + 1, Priority.EXTREME));//5?
        LinkedListNode<Task> high = new LinkedListNode<Task>(new Task(7, this.cycles + cycles, Priority.LOW));//7?
