namespace _01.Classroom
{
    ...
    public class Classroom : IClassroom
    {
        ...
        {
            //if (byHeight.OrderByDescending(s => s.Key).SelectMany(s => s.Value).OrderBy(s => s.Name).OrderByDescending(s => s.Town).ToList().Count == 0)
            //{
            //    return new List<Student>();
            //}
            return students.Select(s => s.Value).OrderByDescending(s => s.Height).ThenBy(s => s.Name).ThenByDescending(s => s.Town).ToList();
            //return byHeight.OrderByDescending(s => s.Key).SelectMany(s => s.Value).OrderBy(s => s.Name).OrderByDescending(s => s.Town).ToList();
        }
